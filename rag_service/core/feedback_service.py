...
class FeedbackService:
        ...
        try:
            ...
            if plagiarism_data:
                ...
                if is_ai_generated:
                    ...
                    feedback = self._create_ai_generated_feedback(plagiarism_data)
                    tempalate_used = "Feedback Template for AI Generated Submission"

                # Handle plagiarized submissions
                elif is_plagiarized:
                    result_status = "Success - Flagged"
                    feedback = self._create_plagiarism_feedback(plagiarism_data)
            ...
            feedback["translation_language"] = assignment_context["student"].get(
                "language", "English"
            )
    ...
    async def process_feedback(
        ...
        try:
            ...
            feedback_request.db_set(
                "completed_at", datetime.now(), update_modified=True
            )
            feedback_request.db_set("model_used", model_used, update_modified=True)
            feedback_request.db_set(
                "template_used", template_used, update_modified=True
            )
            ...
            try:
                if "feedback_request" in locals() and feedback_request:
                    ...
                    feedback_request.db_set(
                        "error_log", error_msg, update_modified=True
                    )
                    frappe.db.commit()
    ...
    async def _update_result_status(
        self, feedback_request_id: str, status: str, error_message: str = None
    ):
        ...
        frappe.db.set_value(
            "Feedback Request", feedback_request_id, update_data, update_modified=True
        )
        frappe.db.commit()
    ...
    def validate_feedback_structure(
        self, feedback: Dict, expected_format: Dict
    ) -> Dict:
    ...
    def create_error_feedback(self, error_msg: str) -> Dict:
        ...
        feedback = {
            ...
            "strengths": [
                f"Your submission was received successfully but system encountered an error during processing.{error_msg}"
            ],
            "areas_for_improvement": [
                "No issues identified with your submission - this appears to be a technical problem"
            ],
            "learning_objectives_feedback": [
                "Unable to evaluate due to system error - please resubmit"
            ],
            "final_grade": 0,
